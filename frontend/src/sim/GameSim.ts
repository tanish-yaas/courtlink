...
export class GameSim {
  ...
  private launchServe(server: Side, swing: PendingSwing) {
    // Directional serve (air-hockey style): the flick direction aims it and the
    // flick speed sets depth. We only force it to land in the opponent's court
    // BEYOND their kitchen line (a legal serve) — otherwise it's freely aimable.
    ...
    const oppMinX = server === 'A' ? KITCHEN_B_MAX_X + 0.6 : 0.8;
    const oppMaxX = server === 'A' ? COURT_LENGTH - 0.8 : KITCHEN_A_MIN_X - 0.6;
    tx = clamp(tx, oppMinX, oppMaxX);
    ty = clamp(ty, 0.8, COURT_WIDTH - 0.8);
